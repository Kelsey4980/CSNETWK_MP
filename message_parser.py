...
class LSNPMessage:
    ...
    def validate_token(self) -> bool:
        ...
        try:
            parts = token.split('|')
            if len(parts) != 3:
                self.validation_errors.append("Invalid token format")
                return False
...
class MessageParser:
    ...
    def _extract_fields(self, raw_message: str) -> Dict[str, str]:
        """Extract key-value pairs from raw message"""
        fields = {}
    ...
    def _validate_message(self, message: LSNPMessage):
        ...
        for field in required_fields:
            if field not in message.fields:
                message.validation_errors.append(f"Missing required field: {field}")
                message.is_valid = False
        
        # Additional validation based on message type
        if message.message_type == MessageType.POST:
            if "CONTENT" in message.fields and len(message.fields["CONTENT"]) > 500:
                message.validation_errors.append("Post content too long")
                message.is_valid = False
